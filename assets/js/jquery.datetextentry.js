...
(function ($) {
    ...
    var DateTextInput = function (options) {
        ...
        this.$input = $('<input type="text" value="" />')
            ...
            .keydown(function (e) { 
                ...
                setTimeout(function () { input.keydown(e); }, 2); 
            })
            .keyup(function (e)   { 
            ...
            .on("paste", function(event) {
                ...
                // console.log(isValidDate(pastedText));

                var dateformat=isValidDate(pastedText);
                console.log(dateformat);
                console.log(typeof dateformat);

                if (typeof dateformat == 'object') {
                  $(this).closest('span').find('.jq-dte-day').val(dateformat.Date).removeClass('hint');
                  $(this).closest('span').find('.jq-dte-month').val(dateformat.Month).removeClass('hint');
                  $(this).closest('span').find('.jq-dte-year').val(dateformat.Year).removeClass('hint');  

                  $(this).closest('span').find('.jq-dte-day').removeClass('error');
                  $(this).closest('span').find('.jq-dte-month').removeClass('error');
                  $(this).closest('span').find('.jq-dte-year').removeClass('error');
                  $(this).closest('span').find('.jq-dte-errorbox').css('display','none');   

                  var formatDt=dateformat.Month + '/' + dateformat.Date + '/' + dateformat.Year;
                  $(this).closest('span').parent().next('input.DtHidden').val(formatDt);
                  $(this).closest('span').prev('input').val(formatDt);

                  $(this).closest('span').find('.jq-dte-year').trigger('focusout') ;
                        
                }else{
                  $(this).closest('span').find('.jq-dte-day').val(dateformat.Date).addClass('hint');
                  $(this).closest('span').find('.jq-dte-month').val(dateformat.Month).addClass('hint');
                  $(this).closest('span').find('.jq-dte-year').val(dateformat.Year).addClass('hint');  

                  $(this).closest('span').next('span').next('span.jq-dte-errorbox').html('Invalid Date'); 
                  $(this).closest('span').next('span').next('span.jq-dte-errorbox').css('display','block');       
                  $(this).closest('span').next('span').next('span.jq-dte-errorbox').css('position','absolute');       
                  $(this).closest('span').next('span').next('span.jq-dte-errorbox').css('top','30px');   
    ...
    function isValidDate(s) {
        ...
        if (dateFormat.test(s)) {
            ...
            if (testDate.getMonth()!=dateArray[0] || testDate.getDate()!=dateArray[1] || testDate.getFullYear()!=dateArray[2]) {
              return false;
            } else {
              dateArray[0]++;
              var tdate= dateArray[1] > 9 ? dateArray[1] : '0' + dateArray[1];
              var tmonth= dateArray[0] > 9 ? dateArray[0] : '0' + dateArray[0];
              var dateobj={'Month':tmonth, 'Date':tdate, 'Year':dateArray[2]};
              return dateobj;
              }
        } else {
            return false;
